...
if(isset($_POST['page']))
{
	if($_POST['page'] == 'login')
	{
		if($_POST['action'] == 'check_login')
		{
			...
			$url = '';
			$object->data = array(
				':student_username'	=>	$_POST["student_name"]
			);

			$object->query = "
				SELECT * FROM student_soes 
				WHERE student_name = :student_username
			";
	...
	if($_POST['page'] == 'exam')
	{
		if($_POST["action"] == 'fetch')
		{
			...
			WHERE exam_mode='MCQ' AND exam_status != 'Pending' AND exam_class_id = '".$_POST["class_id"]."' 
			";
		...
		if($_POST["action"] == 'fetch1')
		{
			...
			$main_query = "
			SELECT * FROM exam_soes 
			WHERE exam_mode='Subjective' AND exam_status != 'Pending' AND exam_class_id = '".$_POST["class_id"]."' 
			";
			...
			foreach($result as $row)
			{
				...
				$sub_array[] = '<a href="view_exam1.php?ec='.$row["exam_code"].'">View</a>';
		...
		if($_POST["action"] == 'fetch_timetable1')
		{
			$object->query = "
			SELECT * FROM subjective_subject
			INNER JOIN subject_soes 
			ON subject_soes.subject_id = subjective_subject.subject_id 
			WHERE subjective_subject.exam_id = '".$_POST["exam_id"]."'
			";
	...
	if($_POST['page'] == 'view_subject_exam')
	{
			...
			foreach($result as $row)
			{
				...
				$output .= '
				  	<div align="center">
				   		...
				   		<button type="button" name="next" class="btn btn-success btn-lg submit" >Submit</button>
		...
		if($_POST['action'] == 'submission')
		{
			...
			$object->query = "
			Insert into  student_ec ('Student_id','Exam_id',''Subject_id,'exam_status')
			values (:student_id,':exam_id','exam_subject_id',':ans')

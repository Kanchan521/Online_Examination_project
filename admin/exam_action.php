...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		if(isset($_POST["search"]["value"]))
		{
			...
			$search_query .= 'OR exam_soes.exam_mode LIKE "%'.$_POST["search"]["value"].'%" ';
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = html_entity_decode($row["exam_mode"]);
	...
	if($_POST["action"] == 'Add')
	{
		...
		else
		{
			...
			(exam_title,exam_mode, exam_class_id, exam_duration, exam_status, exam_created_on, exam_code) 
			VALUES (:exam_title,:exam_mode, :exam_class_id, :exam_duration, :exam_status, :exam_created_on, :exam_code)
			";
	...
	if($_POST["action"] == 'Edit')
	{
		...
		$object->query = "
		UPDATE exam_soes 
		SET exam_title = :exam_title, exam_mode=:exam_mode,

...
if(isset($_POST["action"]))
{
	...
	{
		$order_column = array('exam_soes.exam_title', 'subject_soes.subject_name', 'subjective_subject.subject_exam_datetime', 'subjective_subject.subject_total_question');

		$output = array();

		$main_query = "
		SELECT * FROM subjective_subject 
		...
		if(isset($_POST["search"]["value"]))
		{
			...
			$search_query .= 'OR subjective_subject.subject_exam_datetime LIKE "%'.$_POST["search"]["value"].'%" ';
			$search_query .= 'OR subjective_subject.subject_total_question LIKE "%'.$_POST["search"]["value"].'%" ';
		...
		{
			$order_query = 'ORDER BY subjective_subject.exam_sub_id DESC ';
		}
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '
			<div align="center">
			<button type="button" name="edit_button" class="btn btn-warning btn-circle btn-sm edit_button" data-id="'.$row["exam_sub_id"].'"><i class="fas fa-edit"></i></button>
			&nbsp;
			<button type="button" name="delete_button" class="btn btn-danger btn-circle btn-sm delete_button" data-id="'.$row["exam_sub_id"].'"><i class="fas fa-times"></i></button>
			</div>
			';
			$data[] = $sub_array;
		}
	...
	if($_POST["action"] == 'Add')
	{
		...
		$object->query = "
		INSERT INTO subjective_subject
		(exam_id, subject_id, subject_total_question,  subject_exam_datetime, subject_exam_code) 
		VALUES (:exam_id, :subject_id, :subject_total_question,  :subject_exam_datetime, :subject_exam_code)
		";
	...
	if($_POST["action"] == 'fetch_single')
	{
		$object->query = "
		SELECT * FROM subjective_subject
		WHERE exam_sub_id = '".$_POST["exam_subject_id"]."'
		";
	...
	{
		$object->query = "
		DELETE FROM subjective_subject
		WHERE exam_sub_id = '".$_POST["id"]."'
		";
